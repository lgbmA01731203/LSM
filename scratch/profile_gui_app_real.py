import os
import cv2
import sys
import time
import numpy as np

# Add parent directory to sys.path
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from main import LSMApp
import tkinter as tk

def profile_gui_real():
    print("=== Profiling LSM GUI Application Update Loop (Real Rendering) ===")
    
    app = LSMApp()
    app.withdraw()
    
    # Mock winfo_width and winfo_height to simulate actual layout dimensions
    app.camera_frame.winfo_width = lambda: 800
    app.camera_frame.winfo_height = lambda: 600
    
    # Pre-populate pipeline with a dummy frame and features
    dummy_frame = np.zeros((480, 640, 3), dtype=np.uint8)
    # Draw a white rectangle to simulate a hand so features has some values
    cv2.rectangle(dummy_frame, (100, 100), (300, 300), (255, 255, 255), -1)
    
    dummy_features = np.zeros(126)
    dummy_features[10:20] = 0.5  # Simulate hand features
    
    with app.pipeline._out_lock:
        app.pipeline._out_frame = dummy_frame
        app.pipeline._out_features = dummy_features
        
    durations = []
    
    print("Running 100 GUI updates with rendering enabled...")
    for i in range(100):
        t_start = time.time()
        
        # Override self.after to be a no-op
        original_after = app.after
        app.after = lambda ms, func: None
        
        app.update_frame()
        
        app.after = original_after
        durations.append(time.time() - t_start)
        
    app.pipeline.stop()
    app.destroy()
    
    avg_duration = np.mean(durations)
    print(f"Average time per update_frame: {avg_duration * 1000:.2f} ms")
    print(f"Theoretical GUI max frame rate: {1.0 / avg_duration:.2f} FPS")
    
if __name__ == "__main__":
    profile_gui_real()
