...
class MPHandPose:
    def __init__(self, modelPath, confThreshold=0.8, backendId=0, targetId=0):
        ...
        # Determine execution providers for ONNX Runtime (GPU -> CPU fallback)
        try:
            # First try GPU (DirectML bypasses CUDA version issues on Windows)
            self.sess = ort.InferenceSession(self.model_path, providers=["DmlExecutionProvider", "CUDAExecutionProvider"])
            print(f"[MPHandPose] Successfully loaded model on GPU: {self.model_path}")
        except Exception as e:
            # Fall back to CPU
            print(f"[MPHandPose] GPU loading failed ({e}). Falling back to CPU.")
    ...
    def setBackendAndTarget(self, backendId, targetId):
        # Kept for compatibility but no-op
        self.backend_id = backendId
        self.target_id = targetId
    ...
    def infer(self, image, palm):
        ...
        output_blob = self.sess.run(self.output_names, {self.input_name: input_blob})
